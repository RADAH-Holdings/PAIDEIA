# Paideia — Functional Specification (MVP v2.0)

> Paideia · MVP · Functional Specification Document · v2.0

- **Document ID:** PAI-FSD-001
- **Version:** 2.0 — Draft
- **Supersedes:** PAI-FSD-001 v1.0
- **New in v2.0:** Stack · Modules · Rendering Engine
...
```
Module Interaction Map — MVP Auth & Identity (M1)→Account Management (M2)→Course Brief (M3) ↓ Teacher enrolls student (Admin as fallback) Enrollment (M4)→Session Orchestration (M5) ↓ Student opens session Context Assembly (M6)→AI Agent (M7) ↓ Returns lesson JSON + assessment JSON Rendering Engine (M8)→Student sees lesson ↓ Assessment complete Session History Store (within M5)→Next session: feeds M6
```

## 05 — Technology Stack Decisions
...
| Auth integration quality | Clerk was a fixed decision. The backend and frontend frameworks must have first-class Clerk support to avoid building a JWT validation layer from scratch. |
...
| Clerk identity bridge | Trying to manage users in both Clerk and Django creates sync drift — accounts that exist in one but not the other. | Clerk owns identity. Django stores clerk_user_id + role + school_id only. Custom DRF authenticator validates JWT and upserts local record. |
...
### 5.4 Deployment Architecture (Railway)

**Production Architecture — All on Railway**

```
Production Architecture — All on Railway SERVICE 1 Next.js Frontend Railway Node.js service. App Router. Clerk middleware. SSE client for streaming lessons. Static assets served via Railway CDN. SERVICE 2 Django API Railway Python service. Gunicorn + Uvicorn workers (ASGI). django-cors-headers scoped to frontend URL. Environment variable for model version. SERVICE 3 PostgreSQL Railway managed Postgres. Daily backups. Private networking between Django and DB (no public endpoint needed). psycopg3 driver. Student browser→Next.js (Railway)→ JWT →Django API (Railway) ↓ async view triggered Anthropic API← stream →Django streaming endpoint→ stream →Next.js fetch reader ↓ lesson stored + assessment generated concurrently PostgreSQL (Railway)← read/write →Django ORM (psycopg3)
```

> **Environment Variables** — The following must be set in Railway environment config and never hardcoded: ANTHROPIC_API_KEY, ANTHROPIC_MODEL_VERSION, CLERK_SECRET_KEY, CLERK_PUBLISHABLE_KEY, DATABASE_URL, DJANGO_SECRET_KEY, ALLOWED_HOSTS, CORS_ALLOWED_ORIGINS. Rotating ANTHROPIC_MODEL_VERSION without a code deploy is a deliberate design capability.

## 06 — Module 1 — Auth & Identity

Authentication is owned entirely by Clerk. Django's role is validation and authorisation only. The identity bridge pattern described in Section 5.2 applies to every authenticated request in the system.

### 6.1 Django Authentication Class

**authentication.py — Custom DRF Authenticator**
...
class ClerkJWTAuthentication(BaseAuthentication):
    def authenticate(self, request):
        ...
        # Upsert local user on first valid request. ALL NOT NULL columns
        # must be populated from JWT claims — name, email, role, school_id —
        # or the INSERT violates the users table constraints.
        ...
        user, _ = User.objects.get_or_create(
            ...
            defaults={
                ...
                'email':     payload['email'],
                'role':      meta['role'],          # set at account creation
                'school_id': meta['school_id'],
            }
        )
        return (user, token)

# Role enforcement via DRF Permission classes — not Clerk metadata
...
| --- | --- | --- |
| AUTH-01 | All API endpoints require a valid Clerk JWT in the Authorization header. Requests without a token return 401. Expired tokens return 401 with a message that prompts re-authentication. | MUST |
| AUTH-02 | The ClerkJWTAuthentication class is set as the default authentication class in DRF settings. No viewset should need to declare authentication explicitly. | MUST |
| AUTH-03 | On a validated request from a Clerk user not yet in the local database, a user record is created via get_or_create. All NOT NULL columns must be populated from the verified JWT: name and email from the Clerk identity claims, role and school_id from Clerk public_metadata (both set at account creation by the admin flow per ACC-01). A creation that cannot populate these fields fails loudly rather than inserting an invalid row. | MUST |
| AUTH-04 | All viewsets apply the IsSameSchool permission. A student from School A must never be able to read, write, or infer the existence of data belonging to School B, even with a valid JWT. | MUST |
| AUTH-05 | On the Next.js side, Clerk middleware is configured to protect all routes except /sign-in and /sign-up. Role-based route groups (/admin/*, /teacher/*, /student/*) are guarded by layout-level auth checks comparing the user's role from Clerk metadata. | MUST |
| AUTH-06 | Passwords, password resets, session expiry, and device management are fully delegated to Clerk. Django has no password-related code. | MUST |

## 07 — Module 2 — Account Management

Account creation is admin-only. No self-registration. When an admin creates an account, the system creates the Clerk user via the Clerk Backend API and simultaneously creates the local Django user record with the assigned role and school_id stored in Clerk public_metadata.

| Req ID | Requirement | Priority |
| --- | --- | --- |
| ACC-01 | Admin creates a teacher or student account by providing name, email, and role. System calls Clerk Backend API to create the Clerk user, sets a temporary password, stores school_id and role in Clerk public_metadata, and creates the local user record. Clerk sends the welcome/temp password email. | MUST |
| ACC-02 | On first login, Clerk enforces password reset before the user reaches any Paideia page. This is configured in Clerk dashboard settings — no application code required. | MUST |
| ACC-03 | Admin can deactivate any account (teacher or student). This calls the Clerk Backend API to ban the user (preventing login) and sets is_active=False on the local record. Data is retained in full. | MUST |
...
### 8.1 Course Brief Fields

- `course_title` (VARCHAR(200)) — Display name. Shown in all dashboards. E.g. "Biology — Year 10, Term 2".
- `subject` (VARCHAR(100)) — Subject area. Passed verbatim to agent system prompt. E.g. "Biology", "Mathematics".
- `target_level` (VARCHAR(100)) — Free text. Agent uses this to calibrate vocabulary and depth. E.g. "Year 10", "Age 12–13".
- `learning_outcomes` (TEXT (4000)) — What students must know by course end. Agent's primary success metric. Bullet list recommended. Minimum 100 chars enforced.
- `topic_sequence` (TEXT (4000)) — Rough topic order. Agent fills gaps. E.g. "1. Cell biology 2. Genetics 3. Evolution". Minimum 80 chars.
- `exam_context` (TEXT (2000)) — Optional. Exam being prepared for. Agent biases toward exam-relevant depth. E.g. "WAEC Biology — past topics include..."
- `special_instructions` (TEXT (2000)) — Optional. Hard constraints for agent. E.g. "Do not cover evolution — teacher-led only." Agent treats these as absolute rules.
- `approximate_lessons` (INTEGER) — Guides agent on pacing. Range 10–120. Stored and passed to agent as pacing context. Not a hard cap.
...
## 10 — Module 5 — Session Orchestration
...
### 12.1 Lesson Generation — Prompt & Output
...
### 13.1 Rendering Engine Architecture
...
### 13.4 Renderer 2 — Interactive Sandbox
...
### 13.5 Renderer 3 — Assessment State Machine
...
### 13.7 Library Whitelist & Subject Mapping
...
### 15.1 History Entry Written Per Completed Session

- `session_number` (Integer) — Sequential within enrollment. Starts at 1. Never reused.
- `lesson_title` (String) — Title of the generated lesson.
- `key_concepts_covered` (String[]) — The 3–5 key concept terms from the lesson metadata.
- `had_interactive` (Boolean) — Whether this lesson included an interactive block. Tracked for quality analysis.
- `assessment_outcome` (Enum) — "strong" (4/4), "adequate" (3/4), "needs_reinforcement" (≤2/4), "not_taken".
- `concepts_missed` (String[]) — Key concepts for which the student answered incorrectly. Empty array if strong/adequate.
- `time_on_lesson_secs` (Integer) — Time from lesson display to "Start Assessment" click. Stored but not used in MVP generation context.
- `completed_at` (Timestamp) — When the session reached COMPLETE state.
...
-- All human actors. Role determines access. Clerk owns auth — we own authorisation.
TABLE users {
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid()
  school_id        UUID NOT NULL REFERENCES schools(id)
  clerk_user_id    VARCHAR(255) UNIQUE NOT NULL   -- Clerk's user ID, used as join key
  name             VARCHAR(200) NOT NULL
...
| Security | All API endpoints require a valid Clerk JWT. Student data is only accessible to: the student themselves, their course teacher, and the school admin. No cross-school data access at any layer. | MUST |
