# Paideia — Technical Specification (MVP v1.0)

> Paideia · MVP · Technical Specification Document

- **Document ID:** PAI-TSD-001
- **Version:** 1.0 — Draft
- **Pairs With:** PAI-FSD-001 v2.0
- **Scope:** MVP — Core Loop
- **Audience:** Engineering
- **Classification:** Internal — Confidential

## 01 — Purpose & Document Map
...
> **Governing Principle — Interfaces First** — The central design rule of this TSD is that every boundary between two components has an explicit, versioned contract. The browser, the Next.js frontend, the Django API, the Clerk service, the Anthropic API, and the PostgreSQL database each communicate only through defined contracts (Section 13). No component reaches across a boundary in an undefined way. This is what makes the system buildable by more than one person and debuggable when something breaks.

### 1.1 The Six Boundaries

The MVP has exactly six interface boundaries. Every one is contracted in Section 13. If a behaviour is not described at the boundary it crosses, it is undefined and must not be relied upon.

| # | Boundary | Protocol | Contract |
| --- | --- | --- | --- |
| B1 | Browser ↔ Next.js | HTTPS, SSE | Next.js pages and route handlers; Server-Sent Events for lesson streaming |
| B2 | Next.js ↔ Django API | HTTPS REST + SSE, Bearer JWT | The API Contract (Section 7) — JSON request/response, SSE for session generation |
| B3 | Next.js ↔ Clerk | HTTPS, Clerk SDK | Clerk-managed: sign-in, session, JWT issuance |
| B4 | Django ↔ Clerk | HTTPS, Clerk Backend API | JWT verification; user create/ban operations |
| B5 | Django ↔ Anthropic | HTTPS, Anthropic SDK | Async streaming (lesson) + async non-streaming (assessment) |
| B6 | Django ↔ PostgreSQL | TCP, psycopg3 async | Django ORM async; private Railway networking |

## 02 — Architecture Overview

Paideia MVP is a three-service system on Railway: a Next.js frontend, a Django API backend, and a PostgreSQL database. It integrates two external services — Clerk for identity and the Anthropic API for content generation. The architecture is deliberately conventional; the novelty of the product is in the AI agent loop, not in the infrastructure.

**System Architecture — Runtime View**

```
System Architecture — Runtime View Student / Teacher / Admin Browser │ HTTPS · B1 Next.js 14 — Railway Service 1↔ B3 ↔Clerk (identity) │ HTTPS REST + SSE · Bearer JWT · B2 Django 5 API — Railway Service 2 ├─ B4 → Clerk Backend API (JWT verify, user ops) ├─ B5 → Anthropic API (async stream + async call) └─ B6 → PostgreSQL — Railway Service 3 (psycopg3, private net)
```
...
| Identity ownership | Clerk owns identity; Django owns authorization | No password code in Django. Django stores only clerk_user_id + role + school. Single source of truth per concern. |
...
| --- | --- | --- |
| M1 Auth & Identity | Django + Next.js | DRF authentication class (Django) + Clerk middleware (Next.js) |
...
### 3.2 Frontend — Next.js Service

### 3.3 Infrastructure & External
...
│   │   ├── authentication.py # ClerkJWTAuthentication (DRF)
│   │   ├── permissions.py    # IsAdmin, IsTeacher, IsCourseOwner, IsSameSchool
│   │   ├── clerk.py          # Clerk Backend API wrapper
│   │   ├── serializers.py
│   │   └── views.py
...
│   │   ├── layout.tsx        # ClerkProvider, global KaTeX CSS
...
### 5.1 Schema with Indices & Constraints
...
users
  ...
  INDEX  ix_users_clerk         (clerk_user_id)    -- auth lookup, every request
...
## 06 — Backend Architecture — Django
...
| --- | --- | --- | --- |
| accounts | Schools, users, auth, Clerk bridge | ClerkJWTAuthentication, permission classes, Clerk wrapper | — (base) |
...
| CLERK_SECRET_KEY | Clerk Backend API + JWT verification | — |
...
| Authentication | Every endpoint requires Authorization: Bearer <clerk_jwt>. No anonymous endpoints. |
...
**`GET /api/v1/me`** — _Any authenticated_

Returns the current user's Paideia profile, resolved from the Clerk JWT. The frontend calls this once after sign-in to determine which role interface to route into. If the user is new (first valid request), the local record is created here per FSD AUTH-03.
...
**`POST /api/v1/admin/users`** — _Admin only_

Creates a teacher or student. Calls the Clerk Backend API to create the Clerk user with a temporary password, writes school_id and role into Clerk public_metadata, and creates the local user record (FSD ACC-01). Clerk sends the welcome email.
...
**`GET /api/v1/admin/users`** — _Admin only_

Paginated list of all users in the admin's school with status (FSD ACC-04). Optional ?role= and ?status= filters.

**`PATCH /api/v1/admin/users/{id}/deactivate`** — _Admin only_

Deactivates a user. Bans the Clerk user and sets is_active=false locally (FSD ACC-03). If the target is a teacher, the response includes their active courses so the admin UI can prompt for reassignment (FSD ACC-05). Courses are not modified by this call.
...
**`POST /api/v1/admin/courses/{id}/reassign`** — _Admin only_
...
**`GET /api/v1/courses`** — _Teacher / Admin_
...
**`PATCH /api/v1/courses/{id}`** — _Course owner_
...
**`POST /api/v1/courses/{id}/enrollments`** — _Course owner / Admin_
...
**`GET /api/v1/courses/{id}/enrollments`** — _Course owner / Admin_

Course roster — active enrollments with student name and session count.

**`DELETE /api/v1/enrollments/{id}`** — _Course owner / Admin_
...
### 7.6 Sessions — The Core Endpoints

These endpoints drive the core loop. The first is the only streaming endpoint in the system.

**`POST /api/v1/enrollments/{id}/sessions`** — _Student (own enrollment)_
...
**`GET /api/v1/sessions/{id}/lesson`** — _Session's student / Course owner_
...
**`GET /api/v1/sessions/{id}/assessment`** — _Session's student_
...
**`POST /api/v1/sessions/{id}/responses`** — _Session's student_
...
**`GET /api/v1/courses/{id}/progress`** — _Course owner / Admin_
...
**`GET /api/v1/enrollments/{id}/history`** — _Enrollment's student / Course owner / Admin_
...
**`GET /api/v1/students/me/courses`** — _Student_
...
Authentication answers "who is this request from" — owned by Clerk. Authorization answers "may they do this" — owned by Django. The two are deliberately separate concerns implemented in separate places.

### 8.1 The Token Flow

**Authentication Flow — Sign-in to Authorized Request**

```
Authentication Flow — Sign-in to Authorized Request 1User signs in via Clerk components in Next.js. Clerk validates credentials and issues a short-lived JWT (boundary B3). 2Next.js attaches the JWT as Authorization: Bearer ... on every call to the Django API (boundary B2). 3ClerkJWTAuthentication (DRF default auth class) verifies the JWT signature and expiry against Clerk's keys (boundary B4). 4It resolves the local User record via clerk_user_id, creating it on first sight (FSD AUTH-03). request.user is now the Paideia user. 5DRF permission classes run against request.user.role and the target object's school_id / teacher_id. The view executes only if all pass.
```
...
| IsSameSchool | obj.school_id == user.school_id | Every object-level access — universal guard (FSD AUTH-04) |
...
| Route group | Guard |
| --- | --- |
| (auth)/* | Public. Clerk sign-in / sign-up. |
...
The Clerk middleware (middleware.ts) protects all non-auth routes — an unauthenticated request to any app route is redirected to sign-in before any layout runs.

## 09 — AI Agent Integration Layer

The agent app is the boundary B5 implementation. It is a pure module — given a context dictionary, it returns a validated Lesson or Assessment object. It knows nothing of Django models, HTTP, or sessions.
...
| validators.py | Validates parsed output against pydantic schemas. Validates interactive-block HTML (complete document, no disallowed calls, ≤50k chars, CDN whitelist) per FSD AGENT-05. |
...
### 9.2 Lesson Generation — Internal Sequence

**agent.stream_lesson(context) — Internal Flow**

```
agent.stream_lesson(context) — Internal Flow 1prompts.build_lesson_prompt(context) assembles the system prompt from the context dict (course brief + summarised history). 2client opens AsyncAnthropic.messages.stream(...) with the configured model. An asyncio.wait_for wraps it at AGENT_TIMEOUT_SECONDS. 3Each text delta is yielded upward to the caller (the orchestrator) AND accumulated into a full-response buffer. 4On stream end, parsers.parse_lesson(buffer) extracts the three tag sections. 5validators.validate_lesson(...) checks against the pydantic Lesson schema. Each interactive block's HTML is validated; a failing block is replaced with a text fallback block, not rejected wholesale (FSD AGENT-05). 6A validated Lesson object is returned to the orchestrator alongside the raw response and the plan text for storage.
```
...
### 9.3 Concurrent Assessment Generation
...
```
Lesson Streaming — Full Pipeline fetch() reader (browser)→Next.js route→Django async view ↓ view delegates to orchestrator.run_generation() SessionOrchestrator→agent.stream_lesson()→Anthropic stream ↓ each token flows back up and out as an SSE 'token' event Anthropic→ token →agent→orchestrator→SSE → browser ↓ on stream end parse + validate→store lesson→SSE 'lesson_complete'
```
...
```
POST /enrollments/{id}/sessions — Step by Step 1Async view authenticates the request and loads the enrollment with await ...aget(). Verifies the requesting student owns the enrollment. 2Orchestrator checks for an existing non-complete session. If found → emit session_started (resumed:true), replay the stored lesson, skip generation. If not → continue. 3Create the Session row with status=generating. Emit session_started. 4ContextAssembler reads the course brief and the summarised session_history for this enrollment, builds the context dict. 5Dispatch the concurrent assessment task (Section 9.3). The IncrementalLessonParser begins consuming the raw token stream; once the <meta> section is parsed, emit lesson_meta. 6Stream lesson content — the parser emits only clean text-block markdown as SSE token events; the <plan> and JSON scaffolding are suppressed and never reach the client. A buffer accumulates the full raw response. 7On stream end: parse + validate the full raw buffer into the block array (Section 9.2). Persist the Lesson row including blocks, plan, prompt_context, raw_response. Set status=active. 8Emit lesson_complete with the blocks_url. Await the assessment task. Emit assessment_ready (or the unavailable state). Emit done. Close the stream. The session stays active while the student reads. 9When the student starts the assessment — first question fetch or first answer POST — the orchestrator transitions the session to assessing (FSD SESS-04). A browser close in this state makes the session abandoned and resumable. 10On the final answer, the orchestrator writes the immutable history row and sets status=complete (FSD SESS-05).
```

### 10.3 Pipeline Hazards & Mitigations
...
### 11.1 The Typed API Client

lib/api.ts is the single place the frontend talks to Django. No component calls fetch directly. The client attaches the Clerk JWT, sends and receives JSON, and validates every response against a zod schema from lib/schemas.ts before returning it.
...
No global state library. State is local and scoped: React component state for UI, the Clerk SDK for auth/session, and server-fetched data held in the component that needs it. The assessment engine is the only component with non-trivial state — a state machine (Section 12.4) implemented with useReducer. No browser storage is used anywhere (consistent with the platform constraints).

## 12 — Rendering Engine — Implementation
...
| unknown | — | Returns null, logged client-side |

### 12.2 Renderer 1 — Text & Callout
...
### 12.3 Renderer 2 — Interactive Sandbox
...
| omitted | allow-same-origin | Iframe cannot reach the parent — the key security property |
...
### 12.4 Renderer 3 — Assessment Engine
...
### 12.5 Rendering Engine Data Interface
...
This section makes the governing principle concrete. Each of the six boundaries from Section 1.1 is specified as a contract: what crosses it, in what shape, how errors propagate, and what each side may assume. If a behaviour is not in the contract, it is undefined.

**B1 — Browser ↔ Next.js** (HTTPS · SSE)

- **Transport:** HTTPS for pages and route handlers. A fetch() streaming response (consumed via a ReadableStream reader) over HTTPS for the lesson stream.
- **Crosses:** Rendered React pages; the typed SSE event stream proxied from B2.
- **Auth:** Clerk session cookie, managed entirely by the Clerk SDK. No tokens handled in app code here.
- **Errors:** Page-level error boundaries. A failed stream surfaces as the SSE error event, not an HTTP error.
- **Assumptions:** Browser supports the fetch() streaming response body (ReadableStream) and sandboxed iframes — both covered by the FSD's supported-browser list.

**B2 — Next.js ↔ Django API** (REST + SSE · Bearer JWT)

- **Transport:** HTTPS. JSON for all endpoints except session generation, which is text/event-stream.
- **Crosses:** The full API Contract — Section 7. Every endpoint, request and response shape defined there.
- **Auth:** Authorization: Bearer <clerk_jwt> on every request. Missing/invalid → 401.
- **Errors:** Consistent error envelope (Section 14.1). Frontend validates every response with zod; a contract mismatch throws in development.
- **Versioning:** /api/v1/ prefix. A breaking change is a new version prefix, never a silent shape change.
- **Assumptions:** Django assumes the JWT is genuine only after Clerk verification (B4). Next.js assumes any 2xx body matches its zod schema.

**B3 — Next.js ↔ Clerk** (Clerk SDK)

- **Transport:** HTTPS via @clerk/nextjs. Sign-in/up UI, session management, JWT minting.
- **Crosses:** Credentials in; session + JWT out. role and school_id read from public_metadata for route-group guards.
- **Errors:** Handled by Clerk components. App code never sees raw auth errors.
- **Assumptions:** public_metadata role/school_id are set at account creation (B4) and are authoritative for routing.

**B4 — Django ↔ Clerk** (Clerk Backend API)

- **Transport:** HTTPS via clerk-backend-api, wrapped in accounts/clerk.py.
- **Crosses:** In: JWT verification on every request. Out: create-user and ban-user during account management.
- **Errors:** JWT verify failure → 401. A Clerk API failure during user creation → 502 with the error envelope; no local user row is created (the operation is atomic from the API's perspective).
- **Assumptions:** Clerk is the identity source of truth. Django never trusts a clerk_user_id that has not passed live JWT verification.

**B5 — Django ↔ Anthropic** (Anthropic SDK)

- **Transport:** HTTPS via AsyncAnthropic. Streaming for lessons; non-streaming for assessments.
- **Crosses:** Out: system prompt + context. In: token stream (lesson) / completion (assessment).
- **Contract shape:** Output shape is defined by the prompt and enforced by pydantic validation (Section 9.2). The model is told the exact tag and JSON structure to return.
- **Errors:** API error, timeout, or unparseable output → AgentError/AgentParseError → session deleted, SSE error event. The full raw response is logged.
- **Assumptions:** The agent app assumes nothing about model internals — only that the prompt contract produces parseable output most of the time, and that the failure path is safe when it does not.

**B6 — Django ↔ PostgreSQL** (psycopg3 async)

- **Transport:** TCP over Railway private networking. psycopg v3, async-capable.
- **Crosses:** All persistence. Sync ORM from sync views; async ORM (aget/asave) from the session view.
- **Contract shape:** The schema in Section 5.1. Constraints (unique, FK, cascade) are enforced by the database, not only by application code.
- **Errors:** A constraint violation surfaces as an integrity error → mapped to 409 or 422 by the error layer. A connection failure → 503.
- **Assumptions:** No code path issues a synchronous ORM call inside the async view. Migrations have run before the app version serves traffic (Section 5.2).
...
| Clerk API fails on user create | accounts/clerk.py | No local user row created; atomic failure | Admin sees 502 with retry guidance |
...
| Identity | Clerk-issued short-lived JWTs; verified server-side every request | Forged or stale identity |
...
> **Minors & Generated Content** — The system serves children and delivers AI-generated content to them. The MVP's defence is the sandbox plus server-side validation — content cannot harm the application or leak data. Content appropriateness (a dedicated moderation layer) is explicitly post-MVP per the FSD. This is a known, accepted MVP limitation: the pilot runs with a small, supervised cohort, and teacher diagnostic review (PROG-03) is the human check until the moderation layer ships.
...
### 18.1 Flow A — Student Takes a Lesson

**Flow A — Session start to completion**

```
Flow A — Session start to completion 1Student clicks "Continue" on a course. Next.js issues a streaming fetch() POST to /enrollments/{id}/sessions and reads the response body via a ReadableStream reader (B1→B2). 2Django async view authenticates the JWT (B4), confirms enrollment ownership, hands to SessionOrchestrator. 3Orchestrator finds no open session → creates Session(status=generating) (B6). ContextAssembler reads brief + summarised history (B6). 4Orchestrator dispatches the concurrent assessment task, then calls agent.stream_lesson(context) (B5). 5Anthropic tokens flow agent → orchestrator → SSE token events → browser. The Rendering Engine shows markdown building live. 6Stream ends. Agent parses + validates → Lesson object. Orchestrator persists the lesson (B6), sets status=active, emits lesson_complete. 7Frontend fetches GET /sessions/{id}/lesson, swaps the streamed view for the structured block render — interactive iframes now appear. 8Assessment task has finished; assessment_ready + done emitted. Student reads, clicks "Start Assessment". 9AssessmentEngine fetches questions, posts each answer to POST /sessions/{id}/responses, reveals correctness per question. 10Final answer posted → orchestrator writes the immutable session_history row, sets status=complete, updates enrollment.last_session_at (B6). Summary shown.
```

Touches all six boundaries. Realises FSD SESS-01 through SESS-06, AGENT-01 through AGENT-09, ASMT-*, REND-*.

### 18.2 Flow B — Teacher Creates & Populates a Course

**Flow B — Draft to active class**

```
Flow B — Draft to active class 1Teacher signs in via Clerk (B3). Next.js calls GET /me → role teacher → routes into teacher/*. 2Teacher fills the brief form. POST /api/v1/courses creates a draft course with teacher_id = self (B2→B6). 3Teacher clicks Activate. POST /courses/{id}/activate validates required fields; on pass → status=active. 4Teacher opens enrollment, selects students. POST /courses/{id}/enrollments — IsCourseOwnerOrAdmin confirms ownership, creates/reactivates enrollment rows (B6). 5Students appear on their home screen on next GET /students/me/courses. The course is now live.
```

Realises FSD BRIEF-01/05/07, ENROL-01/05 — and the approved change putting enrollment in the teacher's hands.

### 18.3 Flow C — Teacher Leaves Mid-Term

**Flow C — Deactivation without disruption**

```
Flow C — Deactivation without disruption 1Admin calls PATCH /admin/users/{id}/deactivate on the departing teacher. 2Django bans the Clerk user (B4), sets local is_active=false. Courses are not touched. 3Response returns affected_courses with enrolled counts. Admin UI prompts for reassignment. 4Meanwhile a student in one of those courses starts a session — it works. teacher_id being NULL does not block generation; the course brief and history are intact. 5Admin calls POST /admin/courses/{id}/reassign with a new teacher. teacher_id updates; the new teacher immediately has progress + enrollment access.
```
...
**Phase 01 — Foundation**
Monorepo, both Railway services, PostgreSQL, CI. Django ASGI skeleton with the Uvicorn run command verified. Next.js shell with Clerk wired. The /me endpoint and the identity bridge working end to end.
_Demo: a user signs in and is routed to a role-correct empty dashboard._

**Phase 02 — Accounts & Courses**
...
_Demo: admin creates users; teacher creates a course and enrolls students (Flow B)._

**Phase 03 — The AI Agent — Isolated**
...
_Demo: a fixture context produces a validated Lesson object in a test run._

**Phase 04 — The Core Loop**
...
_Demo: a student streams a generated lesson and a second session adapts from the first (Flow A)._

**Phase 05 — The Rendering Engine**
...
_Demo: a lesson with an interactive visual renders; the assessment runs to a summary._

**Phase 06 — Progress, Resilience & Hardening**
...
_Demo: all FSD acceptance criteria pass; the system is pilot-ready._
